import { get, throwError, merge, now, MINUTE, obj, skip, deferred } from '@tailjs/util';
...
import { v4 } from 'uuid';
import { detectPfx, RequestHandler, EventLogger, bootstrap } from '@tailjs/engine';
...
/** Default logger for {@link NativeHost}. It uses winston internally. */ class DefaultLogger {
    ...
    log(message) {
        ...
        if (this._settings.basePath && this._rootPath !== null) {
            var _message_group;
            const logger = get(this._groupLoggers, (_message_group = message.group) !== null && _message_group !== void 0 ? _message_group : "default", ()=>{
                var _this__rootPath;
                const directory = nodePath__default.join((_this__rootPath = this._rootPath) !== null && _this__rootPath !== void 0 ? _this__rootPath : throwError("Root path has not been initialized."), this._settings.basePath, message.group || "");
                if (!directory.startsWith(this._rootPath)) {
                    throwError(`Invalid path for the group '${message.group}' (${directory}).`);
                }
                ...
                return winston.createLogger({
                    ...
                    transports: [
                        new winston.transports.DailyRotateFile({
                            ...
                            filename: nodePath__default.join(directory, "%DATE%.log.json"),
...
async function decompressZip(data) {
    return new Promise((resolve, reject)=>{
        const entries = [];
        yauzl.fromBuffer(Buffer.from(data), {
            lazyEntries: true
        }, (err, zipfile)=>{
...
class NativeHost {
    ...
    log(message) {
        ...
        if (throttleKey != null) {
            ...
            if (!throttleStats) {
                ...
                throttleStats = throttleStats[0] < now() + MINUTE ? [
                    throttleStats[0],
                    throttleStats[1] + 1,
                    throttleStats[2] + 1
                ] : [
    ...
    _resolvePath(path) {
        ...
        if (path === "js/tail.debug.map.js") {
            try {
                ...
                return nodePath.join(nodePath.dirname(resolved), "iife", path.substring(3));
            } catch (e) {
    ...
    request(request) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            tryCatch(()=>{
                ...
                var _obj;
                const headers = (_obj = obj(request.headers, (kv)=>kv[1] != null ? kv : skip)) !== null && _obj !== void 0 ? _obj : {};
                if (request.body) {
    ...
    async compress(data, algorithm) {
        ...
        const method = algorithm === "br" ? zlib$1.brotliCompress : algorithm === "gzip" ? zlib$1.gzip : null;
    ...
    nextId(scope) {
        ...
        return v4().replaceAll("-", "").match(/[0-9a-fA-F]{1,13}/g).map((value)=>parseInt(value, 16).toString(36)).join("");
    }
...
const createServerContext = (config, initializeOnFirstRequest = true, defaults)=>{
    ...
    const initializeRequestHandler = async ()=>{
        ...
        (_extensions = (_finalConfig = finalConfig).extensions) !== null && _extensions !== void 0 ? _extensions : _finalConfig.extensions = [
            (console.warn("tailjs: No extensions has been configured. Events are only logged to the console."), new EventLogger({
                group: "events",
                minimal: true,
                console: true
            }))
    ...
    const middleware = async (request, response, next, resolveTracker = false)=>{
        try {
            ...
            request[trackerSymbol] = tracker && deferred(async ()=>{
...
/** Logs collected tracker events to the console. */ class ConsoleLogger extends EventLogger {
    constructor(){
        super({
            console: true
        });
...
export { ConsoleLogger, DefaultLogger, NativeHost, addTailJsConfiguration, createServerContext, serve };
