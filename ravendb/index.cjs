        ...
        return util.withRetry(async ()=>{
            ...
            errorHandler: (error)=>{
                return {
                    ...
                    cookies: {},
                    body: util.stringify({
                        Message: util.formatError(error, true)
                    }),
                    error
                };
            }
...
class RavenDbVariableStorage extends RavenDbTarget {
    ...
    async get(keys) {
        const results = [];
        for (const keyBatch of util.batch(keys, 100)){
    ...
    async set(setters) {
        ...
        const requests = setters.map((setter)=>async ()=>{
                ...
                let body = util.parseJson(response.body);
                ...
                createOperation && result.body.Status === "NotModified") {
                    ...
                    body = util.parseJson(response.body);
                    ...
                    if (result.status === 200) {
                        return mapVariableResult(types.VariableResultStatus.Conflict, result);
                    }
                    return mapErrorResult(setter, result);
                }
        ...
        const results = [];
        for (const requestBatch of util.batch(requests, 100)){
    ...
    async query(queries, { page, cursor } = {}) {
        ...
        main: for (const query of queries){
            ...
            const json = util.parseJson(response.body);
            var _json_Results;
...
const queryToRql = (query, { fixed, ifEmpty, append } = {})=>{
    ...
    if (entityIds) {
        ...
        if ((keys === null || keys === void 0 ? void 0 : keys.exclude) != false) {
            ...
            const filters = `${entityIds.map((entityId)=>`startsWith(id(),${util.stringify(keyToDocumentId({
                    scope: query.scope,
                    entityId,
                    key: ""
                }))})`).join(" or ")}`;
        ...
        if (keys) {
            ...
            const keyFilter = entityIds.flatMap((entityId)=>util.map(keys.values, (key)=>`id() ${comparer} ${util.stringify(keyToDocumentId({
                        scope: query.scope,
                        entityId,
                        key
                    }))}`));
        ...
        const comparer = keys.exclude ? "!=" : "==";
        const keyFilter = util.map(keys.values, (key)=>`key ${comparer} ${util.stringify(key)}`).join(" or ");
        if (keyFilter) {
...
exports.RavenDbExtension = RavenDbExtension;
exports.RavenDbTracker = RavenDbTracker;
exports.RavenDbVariableStorage = RavenDbVariableStorage;
