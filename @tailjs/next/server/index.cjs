...
var node = require('@tailjs/node');
var headers = require('next/headers');

const WIRED = Symbol();
const createApi = (config)=>{
    node.addTailJsConfiguration(config);
    const { routeHandler, middleware, resolveTracker } = node.createServerContext({
        matchAnyPath: true
    }, true, {
        resourcesPath: "/tmp"
    });
        ...
        resolveTracker: async (req, res)=>{
            if (!req) {
                ...
                tracker.dispose = async ()=>{
                    const responseCookies = headers.cookies();
...
Object.defineProperty(exports, 'ConsoleLogger', {
  enumerable: true,
  get: function () { return node.ConsoleLogger; }
});
