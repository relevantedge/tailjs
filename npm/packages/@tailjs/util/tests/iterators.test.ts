...
describe("iterators(2)", () => {
  ...
  it("Concatenates", () => {
    expect(concat2(1)).toEqual([1]);
    expect(concat2(null)).toEqual(undefined);
    expect(concat2([null, null])).toEqual(undefined);
    expect(concat2(null, undefined)).toEqual(undefined);
    expect(concat2(1, null, undefined, 2, [3, 4])).toEqual([1, 2, 3, 4]);
    expect(concat2(new Set([1, 2, [3], null, new Set([4, 5, 6])]))).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
  });
});
