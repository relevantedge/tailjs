import { changeIdentifierCaseStyle, forEach, isString, throwError, } from "@tailjs/util";
import { systemTypes } from "../consts";
import { tryParseObjectComposition } from "./tryParseObjectComposition";
export const parseEventTypes = (context) => {
    ...
    forEach([defs.events?.$defs ?? defs.Events?.$defs ?? defs.Events?.properties], (eventDefs) => forEach(eventDefs, ([name, eventSchema]) => {
        ...
        else if (isString(eventSchema.properties?.type?.const) ||
            !eventSchema.properties?.type?.enum.every(isString)) {
            throwError("When a type property is explicitly specified on an implicit event type, it must be either a string const or an enum with string values.");
        }
...
//# sourceMappingURL=parseEventTypes.js.map