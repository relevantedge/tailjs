...
import { ERR_POST_FAILED, F, TAB_ID, VAR_URL, addResponseListener, clean, completeDependency, debug, err, eventSet, hasDependencies, map, navigator, nil, now, push, registerAction, registerSharedState, shift, size, splice, startupLock, tryAcquireRequestLock, } from ".";
const [addPostListener, callPostListeners] = eventSet();
const [addShutdownListener, callShutdownListeners] = eventSet();
export { addPostListener as addQueuePostListener, addShutdownListener as addTerminationListener, };
...
const [post, commit] = registerAction("events", async (events, force) => {
    if (!size(events)) {
        return F;
    }
    await startupLock;
    if (force && !affinity) {
        debug("WARN: Force post downgraded to normal post because affinity has not been set.");
        force = F;
    }
    debug("Post started");
...
let affinity;
const broadcastAffinity = registerSharedState("affinity", () => affinity, (value) => (affinity = value));
const setAffinity = (value) => ((affinity = value), broadcastAffinity(value));
