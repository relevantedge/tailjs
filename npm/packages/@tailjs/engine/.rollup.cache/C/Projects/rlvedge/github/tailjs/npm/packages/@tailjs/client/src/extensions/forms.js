...
import { addViewChangedListener, getComponentContext, getVisibleDuration, onFrame, } from "..";
import { T, addTerminationListener, attr, document, entries, get, getOrSet, getRect, item, listen, map, nil, now, push, replace, scopeAttr, timeout, trackerFlag, trackerPropertyName, undefined, uuidv4, } from "../lib";
const currentValue = Symbol();
export const forms = {
    id: "forms",
    setup(tracker) {
        const formEvents = new Map();
        const getFormFieldValue = (element) => element.selectedOptions
        ...
        const getFormState = (el) => {
            ...
            const refName = scopeAttr(formElement, trackerPropertyName("ref")) || "track_ref";
            const parseElements = () => {
                map(formElement.querySelectorAll("INPUT,SELECT,TEXTAREA"), (el, i) => {
                    ...
                    const field = (state[0].fields[name] ??= {
                        id: el.id || name,
                        name,
                        label: replace(item(el.labels, 0)?.innerText ?? el.name, /^\s*(.*?)\s*\*?\s*$/g, "$1"),
                        activeTime: 0,
            ...
            const state = getOrSet(formEvents, formElement, () => {
                ...
                const ev = {
                    type: "FORM",
                    name: scopeAttr(formElement, trackerPropertyName("form-name")) ||
                ...
                const commitEvent = () => {
                    ...
                    state[3] >= 2 /* FormFillState.Pending */ &&
                        (ev.completed =
                            state[3] === 3 /* FormFillState.Submitting */ || !isFormVisible());
                    push(tracker, cast({
                        ...capturedContext,
                        ...ev,
                        totalTime: now(T) - state[4],
                    }));
                    state[3] = 1 /* FormFillState.Submitted */;
                };
                ...
                listen(formElement, "submit", () => {
                    ...
                    state[3] = 3 /* FormFillState.Submitting */;
                    commitTimeout(() => {
        ...
        const getFieldInfo = (el, [formElement, state] = getFormState(el) ?? [], field = state?.[1].get(formElement)) => field && [state[0], field, formElement, state];
        ...
        const handleLostFocus = () => {
            ...
            if (newValue !== previousValue) {
                ...
                field.filled = T;
                state[3] = 2 /* FormFillState.Pending */;
                entries(form.fields, ([name, value]) => (value.lastField = name === field.name || undefined));
            }
        ...
        let tv0 = 0;
        let t0 = 0;
        const wireFormFields = (document) => {
            document &&
                listen(document, ["focusin", "focusout", "change"], (ev, _, current = getFieldInfo(ev.target)) => {
                    current &&
