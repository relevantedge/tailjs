 ...
 */
export const parseTagString = (input, baseRank, target) => {
    if (!input)
        return [];
    ...
    if (/(?<!(?<!\\)\\)%[A-Z0-9]{2}/.test(input)) {
        try {
            input = decodeURIComponent(input.replace(
...
export const encodeTag = (tag) => tag == null
    ? tag
    : `${tag.ranks.join(":")}${tag.value ? `=${tag.value.replace(/,/g, "\\,")}` : ""}`;
//# sourceMappingURL=parseTagString.js.map