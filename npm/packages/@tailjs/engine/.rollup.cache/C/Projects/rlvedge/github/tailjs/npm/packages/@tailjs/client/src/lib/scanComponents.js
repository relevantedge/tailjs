import { F, T, attr, document, get, hashSet, map, nil, parseInt, push, set, split, str, } from ".";
export function scanAttributes(attributeName, references) {
    if (!references)
        return [];
    const commands = [];
    const seen = hashSet();
    document.querySelectorAll(`[${attributeName}]`).forEach((el) => {
        ...
        while (attr(el, attributeName) != nil) {
            ...
            push(commands, ...map(stack, (data) => ({ add: T, ...data, boundary: el })));
...
//# sourceMappingURL=scanComponents.js.map