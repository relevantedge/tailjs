...
describe("VariableMap", () => {
    it("Can get items, and add items if an initializer is provided..", () => {
        ...
        expect(map(variables)).toEqual([
            [[2 /* VariableScope.Device */, "test"], 10],
            [[0 /* VariableScope.Global */, "test"], 3],
        ]);
    });
    it("Sets", () => {
        const variables = new VariableMap();
        ...
        variables.update({ scope: "device", key: "abc" }, (current) => ((captured = current), 12));
...
//# sourceMappingURL=VariableMap.test.js.map