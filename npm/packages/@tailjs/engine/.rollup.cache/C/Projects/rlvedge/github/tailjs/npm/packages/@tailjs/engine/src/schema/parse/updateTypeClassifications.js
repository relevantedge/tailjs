...
import { mergeBasePropertyClassifications, parseError, updateMinClassifications, } from ".";
export const updateTypeClassifications = (type, seen) => {
    if (!add(seen, type))
        return type;
    ...
    forEach(type.properties, ([, property]) => {
        if (isUndefined(property.classification))
            throw parseError(property.context, "The property's classification is not explicitly specified and cannot be infered from scope.");
        if (isUndefined(property.purposes))
            throw parseError(property.context, "The property's purposes are not explicitly specified and cannot be infered from scope.");
        if (property.required &&
            !validateConsent(type, property)) {
...
//# sourceMappingURL=updateTypeClassifications.js.map