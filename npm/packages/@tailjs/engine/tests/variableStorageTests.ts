...
export const setupStorageTests = (
  factory: StorageFactory,
  { maxItems = 100, timeout = 15_000 }: StorageTestOptions = {}
) => {
  const purgeTestScopes = async (storage: VariableStorage) =>
  ...
  const createStorage = async (reset = true) => {
    ...
    if (reset) {
      await purgeTestScopes(storage);
    }
    return storage;
  };

  const testScopes = ["test", "test1", "test2"];
  const storageTests = {
    testScopes,
    reset: async () => await createStorage(true),
    tests: {
      crud: async () => {
        ...
        expect(
          (
            await storage.get([
              { scope: "test2", key: "test", entityId: "test" },
            ])
          )[0]
        ...
        expect(manySetResults.length).toBe(maxItems);
        ...
        expect(manyGetResults.length).toBe(maxItems + 1);
        ...
        expect(
          manyKeys.filter((key) => seen.has(keyToString(key))).length
        ).toBe(maxItems);
      },
      ...
      query: async () => {
        ...
        expect(
          (await storage.query([{ scope: "test1" }])).variables.length
        ).toBe(3);
        expect(
          (await storage.query([{ scope: "test2" }])).variables.length
        ).toBe(2);
        expect(
          (
            await storage.query([
              { scope: "test1", keys: { values: ["key1"] } },
            ])
          ).variables.length
        ).toBe(2);
        expect(
      ...
      ttl: async () => {
        ...
        expect(current.expires).toBeGreaterThanOrEqual(snapshot.expires!);
      },
    } as const,

    run: (filter?: ("crud" | "query" | "ttl")[]) => {
      afterEach(storageTests.reset);
      if (filter?.includes("crud") !== false) {
        it("Cruds", async () => await storageTests.tests.crud(), timeout);
      }
      if (filter?.includes("query") !== false) {
        it("Queries", async () => await storageTests.tests.query(), timeout);
      }
      if (filter?.includes("ttl") !== false) {
        it("TTLs", async () => await storageTests.tests.ttl(), timeout);
      }
    },
  };
