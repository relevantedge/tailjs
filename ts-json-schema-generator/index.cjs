...
var tsj__namespace = /*#__PURE__*/_interopNamespaceDefault(tsj);
 ...
 */ class AllOfBaseTypeFormatter extends tsj.ObjectTypeFormatter {
    getDefinition(type) {
        ...
        const mergedDefinition = otherBaseTypes.map(([type])=>type).reduce(tsj.getAllOfDefinitionReducer(this.childTypeFormatter), this.getObjectDefinition(type));
...
class PrivacyAnnotatedTypeFormatter extends tsj.AnnotatedTypeFormatter {
    getDefinition(type) {
        ...
        if (privacy || access) {
            const usage = types.parseSchemaDataUsageKeywords([
                privacy,
                access
            ], true);
            usage.classification && (definition[JsonSchemaAnnotations.Classification] = usage.classification);
            usage.purposes && (definition[JsonSchemaAnnotations.Purposes] = types.DataPurposes.parse(usage.purposes, {
                names: true,
 ...
 */ const fixReferences = (schema, schemaId = (_schema_$id = schema.$id) !== null && _schema_$id !== void 0 ? _schema_$id : "")=>{
    ...
    if (util.isArray(schema)) {
        util.forEach(schema, (value)=>fixReferences(value, schemaId));
        return;
...
const generateSchema = (config)=>{
    ...
    const formatter = tsj__namespace.createFormatter(tsjConfig, (fmt)=>{
    ...
    const program = tsj__namespace.createProgram(tsjConfig);
    ...
    const parser = tsj__namespace.createParser(program, tsjConfig);
    const generator = new tsj__namespace.SchemaGenerator(program, parser, formatter, tsjConfig);
    ...
    schema[JsonSchemaAnnotations.Classification] = types.DataClassification.parse((_config_usage_classification = (_config_usage = config.usage) === null || _config_usage === void 0 ? void 0 : _config_usage.classification) !== null && _config_usage_classification !== void 0 ? _config_usage_classification : types.DataClassification.anonymous);
    var _config_usage_purposes;
    schema[JsonSchemaAnnotations.Purposes] = types.DataPurposes.parse((_config_usage_purposes = (_config_usage1 = config.usage) === null || _config_usage1 === void 0 ? void 0 : _config_usage1.purposes) !== null && _config_usage_purposes !== void 0 ? _config_usage_purposes : {}, {
        names: true,
        includeDefault: true
    });
    return schema;
...
exports.AllOfBaseTypeFormatter = AllOfBaseTypeFormatter;
exports.PrivacyAnnotatedTypeFormatter = PrivacyAnnotatedTypeFormatter;
