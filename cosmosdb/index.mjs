...
import { delay, withRetry, group, isArray, batch, now, map, skip, throwError } from '@tailjs/util';
import { VariableResultStatus, extractKey, extractVariable, VariableServerScope } from '@tailjs/types';
...
class CosmosDbTarget {
    ...
    async _execute(action) {
        ...
        return await withRetry(async (retry, previousError)=>{
...
class CosmosDbVariableStorage extends CosmosDbTarget {
    ...
    async set(values) {
        ...
        for (const [scope, scopeSetters] of group(values, (key)=>[
            ...
            for (let setterBatch of batch(scopeSetters, this._batchSize)){
                await this._execute(async (db)=>{
                    ...
                    const operations = map(setterBatch, (setter)=>{
                        ...
                        return [
                            ...
                            (response)=>response.resourceBody && {
                                    status: response.statusCode === 201 ? VariableResultStatus.Created : VariableResultStatus.Success,
                                    ...extractVariable(cosmosItemToVariable(response.resourceBody))
                                }
                        ];
                    });
        ...
        return values.map((setter)=>{
            ...
            var _results_get;
            return (_results_get = results.get(itemId)) !== null && _results_get !== void 0 ? _results_get : throwError(`INV: All setter keys are mapped to operations. ${itemId} was not.`);
        });
    }
    async purge(queries) {
        let n = 0;
                ...
                while(true){
                    const [items, nextToken] = await this._execute(async (db)=>{
                        ...
                        if (projection && projection.length > 0) {
                            const selectFields = [
                                "c.id",
                                ...map(projection, (field)=>field === "id" ? skip : `c.${field}`)
                            ];
    ...
    async renew(queries) {
        ...
        for await (const { scope, results } of this._query(queries, {
            ...
            await this._execute(async (db)=>{
                n += results.length;
                const container = await this._getScopeContainer(db, scope);
                const upserts = map(results, (projection)=>{
                    if (!projection.ttl) {
                        return skip;
                    }
                    return {
...
export { CosmosDbExtension, CosmosDbVariableStorage };
