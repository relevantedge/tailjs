...
const flash = "\x1b[37m";
const format = (text, format = green)=>format + text + reset;
if (process.argv.includes("--help")) {
    ...
    if (![
        ...
        "mjs",
        "ts"
    ].some((ext)=>fs.existsSync(nextConfigFile = "./next.config." + ext))) {
        console.log(format("tail.js: No NextJS config file found in the current directory, no action taken.", blue));
    } else {
        console.log(format(`tailjs: Found the NextJS configuration file '${nextConfigFile}'.\n`));
        const prefix = fs.existsSync("src") ? "./src/" : "./";
        const apiDir = prefix + "app/api/tailjs";
        const useAt = fs.existsSync("tsconfig.json") && fs.readFileSync("tsconfig.json").includes('"@/*":');
        for (const dir of [
            apiDir
        ]){
        ...
        const getImportReference = (from, to)=>useAt ? "@/" + path.posix.relative(prefix, to).replace(".ts", "") : path.posix.relative(from, to).replace(".ts", "");
...
export const { GET, POST } = api;`
            ...
            if (fs.existsSync(file)) {
                console.log(format(`tail.js: ${description} ('${file}') already exists, no action taken.\n`, blue));
            } else {
                fs.writeFileSync(file, content, "utf-8");
                console.log(format(`tail.js: ${description} ('${file}') was added to your project.\n`, blueBold));
            }
        } catch (e) {
            console.error(`tail.js: ${description} ('${file}') could not be created.\n`, e);
        }
        console.log(format(`tail.js: Configuration and routing were added.
...
Please note that @tailjs does currently NOT support turbopack, so you have to use webpack.`, flash));
    }
}
