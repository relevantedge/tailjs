...
/// 
/// Backends are required to respect this consent, yet IT IS NOT THE RESPONSIBILITY OF TAIL-F.JS TO ENFORCE IT since it has no way to know the domain context of the data it relays.
/// 
/// The user&#39;s decision is stored in an essential cookie and updated accordingly with this event. Sending the event with  <see cref="NonEssentialTracking"/>  `false` revokes the consent if already given. The event should ideally be sent from a cookie disclaimer.
/// 
...
) : ITrackedEvent
{
  ...
  /// </summary>
  public string Type => "CONSENT";
  
  
  /// <summary>
  /// Whether the user has consented to non-essential tracking.
  /// </summary>
  public bool NonEssentialTracking { get; set; } = NonEssentialTracking;
